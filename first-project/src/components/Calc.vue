<template>
  <div class="hello">
    ...
    <button v-for="(operand, idx) in operands" @click="calculate(operand)" :key="idx" :name="operand" title="operand">{{ operand }}</button> <!-- Кнопки-операции -->
    <hr />
    ...
    <div class="btn-show">
    <label><input type="checkbox" v-model="showvk" name="keyboard" />Показать экранную клавиатуру</label>
    </div>

    <!--Включить / выключить экранную клавиатуру-->

    <div v-if="showvk" id="showblock">
      ...
      <button :name="Del" @click="delLast(operand)" title="Del">Del</button>
      ...
      <div class="operands">
      <label><input type="radio" value="1" v-model="operch" name="operand1" />Операнд 1</label>
      <label><input type="radio" value="2" v-model="operch" name="operand2" />Операнд 2</label>
      </div>
